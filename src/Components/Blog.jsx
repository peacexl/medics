...
import {
  Box, Grid, Heading, Text, Image, Stack, Button, Tag,
  IconButton, useToast
...
const Blog = () => {
  ...
  useEffect(() => {
    ...
    // ✅ Also store posts globally so Account can access them
    localStorage.setItem("medics_allPosts", JSON.stringify(posts));
  }, []);
  ...
  const toggleFavorite = (id) => {
    ...
    users[sessionEmail] = { ...currentUser, favorites: updatedFavorites };
    ...
    toast({
      title: currentFavorites.includes(id)
        ? "Removed from favorites"
        : "Added to favorites",
      status: currentFavorites.includes(id) ? "info" : "success",
      duration: 1500,
    });
  };
        ...
        {posts.slice(0, visibleCount).map((post) => (
          ...
          >
            ...
            <Stack direction="row" justify="space-between" align="center">
              ...
              <IconButton
                icon={favorites.includes(post.id) ? <FaHeart color="red" /> : <FaRegHeart />}
                onClick={() => toggleFavorite(post.id)}
                variant="ghost"
                aria-label="favorite"
              />
            </Stack>
            <Text mt={2} fontSize="sm" color="rgba(255,255,255,0.6)">
              By {post.author}
            </Text>
          </Box>
        ))}
